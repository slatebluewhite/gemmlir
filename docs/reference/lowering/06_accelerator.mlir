#map = affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>
#map1 = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
#map2 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1 + d3, d2 + d4, d5)>
#map3 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2, d3, d4, d5)>
#map4 = affine_map<(d0, d1, d2, d3) -> (0, d1, d2, d3)>
#map5 = affine_map<(d0, d1, d2, d3) -> (d3)>
module attributes {torch.debug_module_name = "Block"} {
  memref.global "private" constant @__constant_16xf32 : memref<16xf32> = dense<[-0.0932260155, -0.0050244038, -0.0149413012, 0.292073935, 0.0149224252, 0.24778448, 0.249534339, -0.10057988, -0.0420088544, 0.459543973, -0.305139065, -0.245570511, 0.427169263, -0.234537929, -0.161344975, -0.167135388]> {alignment = 64 : i64}
  memref.global "private" constant @__constant_3x3x8x16xi8 : memref<3x3x8x16xi8> = dense<""> {alignment = 64 : i64}
  memref.global "private" constant @__constant_16xi32 : memref<16xi32> = dense<[-1379, -2202, 1304, -2244, 2450, 5210, -5263, 1106, -3975, -6599, -5225, -7063, -1256, 12331, -5055, -496]> {alignment = 64 : i64}
  memref.global "private" constant @__constant_144x16xi8 : memref<144x16xi8> = dense<""> {alignment = 64 : i64}
  memref.global "private" @global_seed : memref<i64> = dense<0>
  func.func @forward(%arg0: memref<1x8x16x16xf32>) -> memref<1x16x8x8xf32> {
    %cst = arith.constant 0xFF800000 : f32
    %cst_0 = arith.constant 0.000000e+00 : f32
    %c0_i8 = arith.constant 0 : i8
    %cst_1 = arith.constant 0.0322127827 : f32
    %c-128_i32 = arith.constant -128 : i32
    %c127_i32 = arith.constant 127 : i32
    %cst_2 = arith.constant 0.0206788406 : f32
    %cst_3 = arith.constant 1.82484782E-5 : f32
    %0 = memref.get_global @__constant_144x16xi8 : memref<144x16xi8>
    %1 = memref.get_global @__constant_16xi32 : memref<16xi32>
    %2 = memref.get_global @__constant_3x3x8x16xi8 : memref<3x3x8x16xi8>
    %3 = memref.get_global @__constant_16xf32 : memref<16xf32>
    %alloc = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x8xi8>
    %alloc_4 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x8xi8>
    linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%arg0 : memref<1x8x16x16xf32>) outs(%alloc_4 : memref<1x16x16x8xi8>) {
    ^bb0(%in: f32, %out: i8):
      %4 = arith.divf %in, %cst_1 : f32
      %5 = math.roundeven %4 : f32
      %6 = arith.fptosi %5 : f32 to i32
      %7 = arith.maxsi %6, %c-128_i32 : i32
      %8 = arith.minsi %7, %c127_i32 : i32
      %9 = arith.trunci %8 : i32 to i8
      linalg.yield %9 : i8
    }
    %alloc_5 = memref.alloc() {alignment = 64 : i64} : memref<1x18x18x8xi8>
    %alloc_6 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x16xi32>
    %alloc_7 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x16xi8>
    gemmlir.conv2d_i8(%alloc_4, %2, %alloc_7) bias(%1 : memref<16xi32>) {act = #gemmlir.act<relu>, gemmlir.no_flush_after, padding = 1 : i64, scale = 0.00203614822 : f32} : (memref<1x16x16x8xi8>, memref<3x3x8x16xi8>, memref<1x16x16x16xi8>)
    %alloc_8 = memref.alloc() {alignment = 64 : i64} : memref<1x18x18x16xi8>
    linalg.map outs(%alloc_8 : memref<1x18x18x16xi8>)
      (%init: i8) {
        linalg.yield %c0_i8 : i8
      }
    %subview = memref.subview %alloc_8[0, 1, 1, 0] [1, 16, 16, 16] [1, 1, 1, 1] : memref<1x18x18x16xi8> to memref<1x16x16x16xi8, strided<[5184, 288, 16, 1], offset: 304>>
    memref.copy %alloc_7, %subview : memref<1x16x16x16xi8> to memref<1x16x16x16xi8, strided<[5184, 288, 16, 1], offset: 304>>
    %alloc_9 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x16xi32>
    %alloc_10 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x3x3x16xi8>
    linalg.generic {indexing_maps = [#map2, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel", "parallel"]} ins(%alloc_8 : memref<1x18x18x16xi8>) outs(%alloc_10 : memref<1x16x16x3x3x16xi8>) {
    ^bb0(%in: i8, %out: i8):
      linalg.yield %in : i8
    }
    %collapse_shape = memref.collapse_shape %alloc_10 [[0, 1, 2], [3, 4, 5]] : memref<1x16x16x3x3x16xi8> into memref<256x144xi8>
    %collapse_shape_11 = memref.collapse_shape %alloc_9 [[0, 1, 2], [3]] : memref<1x16x16x16xi32> into memref<256x16xi32>
    gemmlir.matmul_i8(%collapse_shape, %0, %collapse_shape_11) : (memref<256x144xi8> x memref<144x16xi8>) -> memref<256x16xi32> {accumulate = false, gemmlir.no_flush_after}
    %alloc_12 = memref.alloc() {alignment = 64 : i64} : memref<2x2xf32>
    %alloc_13 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x16xf32>
    linalg.generic {indexing_maps = [#map4, #map4, #map5, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7, %alloc_9, %3 : memref<1x16x16x16xi8>, memref<1x16x16x16xi32>, memref<16xf32>) outs(%alloc_13 : memref<1x16x16x16xf32>) {
    ^bb0(%in: i8, %in_16: i32, %in_17: f32, %out: f32):
      %4 = arith.sitofp %in : i8 to f32
      %5 = arith.sitofp %in_16 : i32 to f32
      %6 = arith.mulf %4, %cst_2 : f32
      %7 = arith.mulf %5, %cst_3 : f32
      %8 = arith.addf %7, %in_17 : f32
      %9 = arith.addf %6, %8 : f32
      %10 = arith.cmpf ugt, %9, %cst_0 : f32
      %11 = arith.select %10, %9, %cst_0 : f32
      linalg.yield %11 : f32
    }
    %alloc_14 = memref.alloc() {alignment = 64 : i64} : memref<1x8x8x16xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_14 : memref<1x8x8x16xf32>)
    linalg.pooling_nhwc_max {dilations = dense<1> : vector<2xi64>, strides = dense<2> : vector<2xi64>} ins(%alloc_13, %alloc_12 : memref<1x16x16x16xf32>, memref<2x2xf32>) outs(%alloc_14 : memref<1x8x8x16xf32>)
    %alloc_15 = memref.alloc() {alignment = 64 : i64} : memref<1x16x8x8xf32>
    linalg.transpose ins(%alloc_14 : memref<1x8x8x16xf32>) outs(%alloc_15 : memref<1x16x8x8xf32>) permutation = [0, 3, 1, 2] 
    memref.dealloc %alloc : memref<1x16x16x8xi8>
    memref.dealloc %alloc_4 : memref<1x16x16x8xi8>
    memref.dealloc %alloc_5 : memref<1x18x18x8xi8>
    memref.dealloc %alloc_6 : memref<1x16x16x16xi32>
    memref.dealloc %alloc_7 : memref<1x16x16x16xi8>
    memref.dealloc %alloc_8 : memref<1x18x18x16xi8>
    memref.dealloc %alloc_9 : memref<1x16x16x16xi32>
    memref.dealloc %alloc_10 : memref<1x16x16x3x3x16xi8>
    memref.dealloc %alloc_12 : memref<2x2xf32>
    memref.dealloc %alloc_13 : memref<1x16x16x16xf32>
    memref.dealloc %alloc_14 : memref<1x8x8x16xf32>
    return %alloc_15 : memref<1x16x8x8xf32>
  }
}


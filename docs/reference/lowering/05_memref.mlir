#map = affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>
#map1 = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
#map2 = affine_map<(d0, d1, d2, d3) -> (0, d1, d2, d3)>
#map3 = affine_map<(d0, d1, d2, d3) -> (d3)>
#map4 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1 + d3, d2 + d4, d5)>
#map5 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2, d3, d4, d5)>
module attributes {torch.debug_module_name = "Block"} {
  memref.global "private" constant @__constant_16xf32 : memref<16xf32> = dense<[-0.0932260155, -0.0050244038, -0.0149413012, 0.292073935, 0.0149224252, 0.24778448, 0.249534339, -0.10057988, -0.0420088544, 0.459543973, -0.305139065, -0.245570511, 0.427169263, -0.234537929, -0.161344975, -0.167135388]> {alignment = 64 : i64}
  memref.global "private" constant @__constant_3x3x8x16xi8 : memref<3x3x8x16xi8> = dense<""> {alignment = 64 : i64}
  memref.global "private" constant @__constant_16xi32 : memref<16xi32> = dense<[-1379, -2202, 1304, -2244, 2450, 5210, -5263, 1106, -3975, -6599, -5225, -7063, -1256, 12331, -5055, -496]> {alignment = 64 : i64}
  memref.global "private" constant @__constant_144x16xi8 : memref<144x16xi8> = dense<""> {alignment = 64 : i64}
  memref.global "private" @global_seed : memref<i64> = dense<0>
  func.func @forward(%arg0: memref<1x8x16x16xf32>) -> memref<1x16x8x8xf32> {
    %cst = arith.constant 0xFF800000 : f32
    %cst_0 = arith.constant 0.000000e+00 : f32
    %c0_i32 = arith.constant 0 : i32
    %c0_i8 = arith.constant 0 : i8
    %cst_1 = arith.constant 0.0322127827 : f32
    %c-128_i32 = arith.constant -128 : i32
    %c127_i32 = arith.constant 127 : i32
    %cst_2 = arith.constant 4.21051845E-5 : f32
    %cst_3 = arith.constant 0.0206788406 : f32
    %cst_4 = arith.constant 1.82484782E-5 : f32
    %0 = memref.get_global @__constant_144x16xi8 : memref<144x16xi8>
    %1 = memref.get_global @__constant_16xi32 : memref<16xi32>
    %2 = memref.get_global @__constant_3x3x8x16xi8 : memref<3x3x8x16xi8>
    %3 = memref.get_global @__constant_16xf32 : memref<16xf32>
    %alloc = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x8xi8>
    linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%arg0 : memref<1x8x16x16xf32>) outs(%alloc : memref<1x16x16x8xi8>) {
    ^bb0(%in: f32, %out: i8):
      %4 = arith.divf %in, %cst_1 : f32
      %5 = math.roundeven %4 : f32
      %6 = arith.fptosi %5 : f32 to i32
      %7 = arith.maxsi %6, %c-128_i32 : i32
      %8 = arith.minsi %7, %c127_i32 : i32
      %9 = arith.trunci %8 : i32 to i8
      linalg.yield %9 : i8
    }
    %alloc_5 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x8xi8>
    memref.copy %alloc, %alloc_5 : memref<1x16x16x8xi8> to memref<1x16x16x8xi8>
    %alloc_6 = memref.alloc() {alignment = 64 : i64} : memref<1x18x18x8xi8>
    linalg.map outs(%alloc_6 : memref<1x18x18x8xi8>)
      (%init: i8) {
        linalg.yield %c0_i8 : i8
      }
    %subview = memref.subview %alloc_6[0, 1, 1, 0] [1, 16, 16, 8] [1, 1, 1, 1] : memref<1x18x18x8xi8> to memref<1x16x16x8xi8, strided<[2592, 144, 8, 1], offset: 152>>
    memref.copy %alloc_5, %subview : memref<1x16x16x8xi8> to memref<1x16x16x8xi8, strided<[2592, 144, 8, 1], offset: 152>>
    %alloc_7 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x16xi32>
    linalg.fill ins(%c0_i32 : i32) outs(%alloc_7 : memref<1x16x16x16xi32>)
    linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : vector<2xi64>, gemmlir.output_scale = 0.020678841220000001 : f64, strides = dense<1> : vector<2xi64>} ins(%alloc_6, %2 : memref<1x18x18x8xi8>, memref<3x3x8x16xi8>) outs(%alloc_7 : memref<1x16x16x16xi32>)
    %alloc_8 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x16xi8>
    linalg.generic {indexing_maps = [#map2, #map3, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7, %1 : memref<1x16x16x16xi32>, memref<16xi32>) outs(%alloc_8 : memref<1x16x16x16xi8>) {
    ^bb0(%in: i32, %in_18: i32, %out: i8):
      %4 = arith.addi %in, %in_18 : i32
      %5 = arith.sitofp %4 : i32 to f32
      %6 = arith.mulf %5, %cst_2 : f32
      %7 = arith.cmpf ugt, %6, %cst_0 : f32
      %8 = arith.select %7, %6, %cst_0 : f32
      %9 = arith.divf %8, %cst_3 : f32
      %10 = math.roundeven %9 : f32
      %11 = arith.fptosi %10 : f32 to i32
      %12 = arith.maxsi %11, %c-128_i32 : i32
      %13 = arith.minsi %12, %c127_i32 : i32
      %14 = arith.trunci %13 : i32 to i8
      linalg.yield %14 : i8
    }
    %alloc_9 = memref.alloc() {alignment = 64 : i64} : memref<1x18x18x16xi8>
    linalg.map outs(%alloc_9 : memref<1x18x18x16xi8>)
      (%init: i8) {
        linalg.yield %c0_i8 : i8
      }
    %subview_10 = memref.subview %alloc_9[0, 1, 1, 0] [1, 16, 16, 16] [1, 1, 1, 1] : memref<1x18x18x16xi8> to memref<1x16x16x16xi8, strided<[5184, 288, 16, 1], offset: 304>>
    memref.copy %alloc_8, %subview_10 : memref<1x16x16x16xi8> to memref<1x16x16x16xi8, strided<[5184, 288, 16, 1], offset: 304>>
    %alloc_11 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x16xi32>
    %alloc_12 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x3x3x16xi8>
    linalg.generic {indexing_maps = [#map4, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel", "parallel"]} ins(%alloc_9 : memref<1x18x18x16xi8>) outs(%alloc_12 : memref<1x16x16x3x3x16xi8>) {
    ^bb0(%in: i8, %out: i8):
      linalg.yield %in : i8
    }
    %collapse_shape = memref.collapse_shape %alloc_12 [[0, 1, 2], [3, 4, 5]] : memref<1x16x16x3x3x16xi8> into memref<256x144xi8>
    %collapse_shape_13 = memref.collapse_shape %alloc_11 [[0, 1, 2], [3]] : memref<1x16x16x16xi32> into memref<256x16xi32>
    linalg.fill ins(%c0_i32 : i32) outs(%collapse_shape_13 : memref<256x16xi32>)
    linalg.matmul ins(%collapse_shape, %0 : memref<256x144xi8>, memref<144x16xi8>) outs(%collapse_shape_13 : memref<256x16xi32>)
    %alloc_14 = memref.alloc() {alignment = 64 : i64} : memref<2x2xf32>
    %alloc_15 = memref.alloc() {alignment = 64 : i64} : memref<1x16x16x16xf32>
    linalg.generic {indexing_maps = [#map2, #map2, #map3, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_8, %alloc_11, %3 : memref<1x16x16x16xi8>, memref<1x16x16x16xi32>, memref<16xf32>) outs(%alloc_15 : memref<1x16x16x16xf32>) {
    ^bb0(%in: i8, %in_18: i32, %in_19: f32, %out: f32):
      %4 = arith.sitofp %in : i8 to f32
      %5 = arith.sitofp %in_18 : i32 to f32
      %6 = arith.mulf %4, %cst_3 : f32
      %7 = arith.mulf %5, %cst_4 : f32
      %8 = arith.addf %7, %in_19 : f32
      %9 = arith.addf %6, %8 : f32
      %10 = arith.cmpf ugt, %9, %cst_0 : f32
      %11 = arith.select %10, %9, %cst_0 : f32
      linalg.yield %11 : f32
    }
    %alloc_16 = memref.alloc() {alignment = 64 : i64} : memref<1x8x8x16xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_16 : memref<1x8x8x16xf32>)
    linalg.pooling_nhwc_max {dilations = dense<1> : vector<2xi64>, strides = dense<2> : vector<2xi64>} ins(%alloc_15, %alloc_14 : memref<1x16x16x16xf32>, memref<2x2xf32>) outs(%alloc_16 : memref<1x8x8x16xf32>)
    %alloc_17 = memref.alloc() {alignment = 64 : i64} : memref<1x16x8x8xf32>
    linalg.transpose ins(%alloc_16 : memref<1x8x8x16xf32>) outs(%alloc_17 : memref<1x16x8x8xf32>) permutation = [0, 3, 1, 2] 
    memref.dealloc %alloc : memref<1x16x16x8xi8>
    memref.dealloc %alloc_5 : memref<1x16x16x8xi8>
    memref.dealloc %alloc_6 : memref<1x18x18x8xi8>
    memref.dealloc %alloc_7 : memref<1x16x16x16xi32>
    memref.dealloc %alloc_8 : memref<1x16x16x16xi8>
    memref.dealloc %alloc_9 : memref<1x18x18x16xi8>
    memref.dealloc %alloc_11 : memref<1x16x16x16xi32>
    memref.dealloc %alloc_12 : memref<1x16x16x3x3x16xi8>
    memref.dealloc %alloc_14 : memref<2x2xf32>
    memref.dealloc %alloc_15 : memref<1x16x16x16xf32>
    memref.dealloc %alloc_16 : memref<1x8x8x16xf32>
    return %alloc_17 : memref<1x16x8x8xf32>
  }
}

